    ...
    <Layout user={user} misc={expStore} page={profile.username == user.username ? 'profile' : 'home'}>
      ...
      <div className={styles.user}>
        ...
        <div className={styles.miscInfo}>
          <p style={{color: "#888"}} className={styles.joinDate}>{profile.flags.early_supporter ? <Icon name="star" width=".9rem" style={{color: "#e2ec56!important"}} /> : ''} <Calender width=".9rem" fill="#888" className={styles.joinDateIcon}></Calender>Joined {moment(profile.joinedTimestamp).format('MMMM YYYY')}</p>
        </div>
      </div>
