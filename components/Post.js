...
const data = ({data, embed, useTwemoji, big=false}) => {
    ...
    if(big) {
        ...
        <div className={postStyle.bigauthor}>
            ...
            <a style={{marginLeft: "4px"}} href={`/p/${data.author.username}`}><span className={postStyle.displayName}>{data.author.displayName}</span>{data.author.verified ? <Badge className={postStyle.badge + ' ' + postStyle.big} width="15px" style={{color: "var(--badge-color)"}}></Badge> : ''}</a>
